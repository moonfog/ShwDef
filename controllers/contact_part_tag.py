...
def tag():
    ...
    if len(contact_part_tag_errors)==0: 
        ...
        form1=FORM(TABLE(TR(TD(LABEL(T('Contact Date :')),_class="w2p_f1"),TD(INPUT(_name='contact_date',_readonly='readonly'))),
                    TR(TD(LABEL(T('Guest :')),_class="w2p_f1"),TD(INPUT(_name='guest_name',_readonly='readonly',_type='text'))),
                    TR(TD(LABEL(T('Subject contact part :')),_class="w2p_f1"),TD(INPUT(_name='contact_part_subject',_readonly='readonly')))),_id='form1') 
        ...
        if(tags_already_added()):
            button=TAG.INPUT(_value=T('Next'),_type="button",_onclick="window.location='%s';"%URL(r=request,c='contact_part',f='overview')) 
        else:
            button=None
